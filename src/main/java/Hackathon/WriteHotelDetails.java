...
public class WriteHotelDetails {
	...
	public void WriteToExcel(String[] name, String[] total_amount, String[] charges_pernight) throws IOException {
    	...
    	cell2.setCellValue("Charges Per Night");
    	...
    	for(int i = 1; i<=3; i++) {
    		...
    		col2.setCellValue(total_amount[x]);
    		
    		Cell col3 = rows.createCell(2);
    		col3.setCellValue(charges_pernight[x]);
		...
		FileOutputStream fout = new FileOutputStream(path + "\\eclipse-workspace\\CalculateTripCost\\Excel_Output\\Hotel_Details.xlsx");
		workbook.write(fout);
		fout.close();
		workbook.close();
